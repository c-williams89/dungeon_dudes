...
class MetallicGolem(Golem):
    ...
    def __init__(self, level_mod: int):
        ...
        if metal_type == "Mithril":
            self.stats_structure = {"Hit Points": (3, 0),
                                    "Strength": (15, 3),
                                    "Agility": (100, 0),
                                    "Intelligence": (0, 0),
                                    "Special": (0, 0)}
            self._gold = 0
            self._experience_points = 100 * (25 * (level_mod - 1))
    ...
    def get_skills_list(self) -> list:
        ...
        if self._metal_type == "Mitrhil":
            skills_list.append("run_away")
    ...
    def double_attack(self) -> CombatAction:
        ...
        msg_1: str = \
            "Iron Golem strikes, dealing <value> physical damage."
        msg_2: str = \
            "Iron Golem lashes out again for <value> physical damage."
    ...
    def spiked_body(self) -> CombatAction:
        ...
        spiked_dmg: int = \
            self.golem_damage(self.modify_damage(self._attack_power * 0.25))
    ...
    # mithril golem
    def run_away(self) -> CombatAction:
        '''Mithril golem escapes the encounter'''
        message: str = "Mithril golem flees!"
        return CombatAction([("Escape", "", "", message)], "")
    ...
    def take_damage(self, damage: int, dmg_type: str, message: str) -> bool:
        ...
        if self._metal_type == "Mithril":
            if choice[True, False]:
                dam = 1
            self.immune = True
        return super().take_damage(dam, dmg_type, message)
    ...
    def attack(self) -> CombatAction:
        ...
        total_dmg = damage + razor_sharp_bonus
        message: str = f"{self.name} Attacks for <value> physical damage"
        if self._metal_type == 'Iron':
            self._prev_phys_dmg = damage

        return CombatAction([("Attack", total_dmg, "Physical", message)], "")
    ...
    def take_turn(self) -> CombatAction:
        ...
        if self._metal_type == "Iron":
            if self.level >= 10:
                ...
                weight.append(1)
        if self._metal_type == "Mithril":
            options.append(self.run_away)
            if self._hit_points == 1:
                weight = [0.25, 0.75]
            else:
                weight = [0.75, 0.25]
        selection = choices(options, weight)[0]
        return selection
