...
class TreasureGolem(Golem):
    ...
    def molten_gold(self, damage, dmg_type):
    ...
    def gem_rain(self) -> CombatAction:
        ...
        msg = (f"{self.name} performs gem rain, increasing its gold "
            "and deals <value> physical damage initially. Gems will continue to fall.")
    ...
    def gold_to_iron(self) -> CombatAction:
        ...
        for element in ['Physical', 'Fire', 'Ice', 'Holy']:
            self._def_modifiers[element] = min(self._def_modifiers[element] + 15, 45)
        self._def_modifiers['Lightning'] = max(self._def_modifiers['Lightning'] - 15, -45)
        self._gold = int(self._gold * 0.85)
        msg = (f"{self.name} transmutes gold to iron, improving its defenses but "
            "making it more vulnerable to lightning.")
    ...
    def attack(self) -> CombatAction:
        ...
        if self._gem_rain_active:
            secondary_dmg : int = self.golem_damage(self.modify_damage(self._attack_power * 0.2))
            secondary_msg : str = (f"{self.name}'s gem rain deals an additional <value> "
                                   f"physical damage")
            return CombatAction([("Attack", damage, "Physical", message),
                                 ("Attack", secondary_dmg, "Physical", secondary_msg)]
                                 , "")
        return CombatAction([("Attack", damage, "Physical", message)], "")
